...
测试按《游戏测试分类调研》组织：`TestFarmGame` 检查六种作物、对应加工、切换移除、交易、市场规则及满地图逻辑压力；`TestWorldMap` 检查地图坐标和镜头边界；`TestMainScene` 检查主场景选种、建造和出售的集成流程。`tests/test_suite.tscn` 在同一个 headless Godot 进程中依次运行这些检查，任一失败都会返回非零退出码。满地图检查会将 16,384 格全部放上现有实体（8,192 块农田、8,192 处加工场地，六种作物均覆盖），推进 50 tick 并打印总耗时与平均耗时。耗时是本机观测值，没有固定通过阈值。
