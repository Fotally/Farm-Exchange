...
镜头或视窗变化时只更新块可见性，已绘制且外观未变的块复用缓存。SyncFromGame 扫描 16,384 格快照，解锁状态、建筑、农田作物或作物阶段发生变化时标记相应块为脏；RemainingTicks 数值不影响地图网格。可见脏块重绘，屏幕外脏块等进入视野时重绘。选中框和菱形边缘单独绘制，选格不重建地块。
