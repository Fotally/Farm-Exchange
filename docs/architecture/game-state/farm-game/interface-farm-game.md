...
| 用途 | 公开成员 | 调用方需要知道的约定 |
| --- | --- | --- |
| 定义 | Crops、GetCrop | 六种作物的名称、场地、时长与独立定价倍率来自[作物表](../../../gameplay/production/crop-growth.md) |
| 状态查询 | GetPlot、GetRawStock、GetProductStock、GetRawPriceCents、GetProductPriceCents | 返回指定格的快照、分类库存和两类当日售价；不暴露可变内部数组 |
