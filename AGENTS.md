...
- 开局地图中心赠送 3 块农田和 2 处配套加工场地；农田以 50% 概率出现 3 块同种或 2+1 两种配置。初始金币为 50.00；空地可直接建造，每座新建筑暂收 10.00 金币。玩家界面暂隐藏时间，原料售价显示“待定”，分别等待 issue #25 和 #27 确定规则。
- Godot 引擎目录：`E:\Godot\Godot_v4.7.2-stable_mono_win64`。
- 用户使用中文；项目文档、设计讨论与代码修改说明使用中文。

# 代码系统结构

- `scripts/gameplay/FarmGame.cs`：游戏状态模块。初始化中心随机农田与配套加工场地，持有固定 128×128 土地、每块农田所选作物、工人轮流工作、tick 与天数、分类库存、交易和建造收费规则；界面通过格子快照、作物定义与经营命令使用它。测试专用的满地图填充方法生成 16,384 个现有实体。
...
- `scripts/ui/Main.cs` 与 `scenes/main.tscn`：场景协调及界面。顶部显示金币、工人及库存/市场入口；底部单一建造入口打开农田或加工场地目录，地图点击执行摆放；右侧实体详情和其他二级窗口可拖动并在本次运行内保留位置。按钮命令和计时器交给 `FarmGame`，再刷新地图、分类库存和已确定的售价。
