...
- 现有六种作物为小麦、玉米、水稻、马铃薯、向日葵、甘蔗。农田独立选种；每种原料由对应场地免费加工，只有加工品能出售。小麦磨坊加工时间为 3 tick；各作物时长和价格倍率以 `docs/生产与交易.md` 为准。每 10 tick 经过 1 天，面粉价格按有界市场曲线在 1.00～20.00 金币间变化，其余加工品按当日面粉价倍率定价。
...
- `scripts/WorldMap.cs`：地图表现模块。按 8×8 格缓存带顶点颜色的地图块网格，镜头移动只更新块可见性，经营变化后同步完整世界快照并重建外观变化的块；独立绘制选中框和地图边缘，换算与选择格坐标，限制镜头。不维护经营规则。
...
- `docs/design/地图系统-分块绘制缓存接口.md`：地图分块缓存接口的输入、输出、依赖、同步流程与经营 tick 独立性；修改地图绘制或同步接口时维护。
